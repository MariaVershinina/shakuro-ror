...
function zilla_enqueue_scripts() {
  ...
  wp_register_script('objectFit', get_template_directory_uri() . '/js/polyfill.object-fit.min.js', '', '0.4.1', true);
  wp_register_script('pictureFill', get_template_directory_uri() . '/js/picturefill.min.js', '', '2.3.1', true);
  ...
  wp_register_script('custom-index', get_template_directory_uri() . '/js/custom-index.min.js');
  ...
  if ( ! is_admin() ) {
    wp_enqueue_script('custom-index');
  }
...
if ( !function_exists( 'zilla_add_async_to_picturefill' ) ) :
/**
 ...
 */
function zilla_add_async_to_picturefill( $url ) {
...
add_filter( 'clean_url', 'zilla_add_async_to_picturefill', 11, 1 );
endif;


if ( !function_exists( 'zilla_url_of_registered_script' ) ) :
/**
 ...
 */
function zilla_url_of_registered_script( $handle ) {
  global $wp_scripts;
...
if ( !function_exists( 'zilla_enqueue_objectfit' ) ) :
/**
 * Print Modernizr.load for objectFit polyfill
 *
 * @since 1.2
 */
function zilla_enqueue_objectfit() {
  if ( zilla_url_of_registered_script( 'objectFit' ) ) :
    printf(
      '<script>
        Modernizr.load({
      ...
      zilla_url_of_registered_script( 'objectFit' )
    );
  endif;
}
endif;
add_action('wp_footer', 'zilla_enqueue_objectfit', 10);
...
if ( ! function_exists( 'zilla_set_thumb_transient' ) ) :
/**
 *
 * @package Storey
 * @since Storey 1.2
 ...
 * Called by zilla_get_thumb_data when no transient is set.
 *
 ...
 */
function zilla_set_thumb_transient( $post_id ) {
...
if ( ! function_exists( 'zilla_reset_thumb_transient' ) ) :
/**
 *
...
add_action( 'save_post', 'zilla_reset_thumb_transient' );
endif;


if ( ! function_exists('zilla_get_thumb_data') ) :
/**
 *
 ...
 * @since Storey 1.2
 *
 * Uses: zilla_set_image_transient()
 *
 * @return array - the responsive image urls and alt tag
 */
function zilla_get_thumb_data( $post_id ) {
 ...
 * @since Storey 1.2
 */
function zilla_post_thumbnail( $post_id ) {
  $thumb_data = zilla_get_thumb_data( $post_id );

  printf(
    '<picture class="thumb-responsive">
      <!--[if IE 9]><video style="display: none;"><![endif]-->
      <source srcset="%1$s" media="(min-width: 1921px)">
      <source srcset="%2$s" media="(min-width: 1260px)">
      <source srcset="%3$s" media="(min-width: 960px)">
      <source srcset="%4$s" media="(min-width: 400px)">
      <source srcset="%5$s">
      <!--[if IE 9]></video><![endif]-->
      <img srcset="%5$s" alt="%6$s">
    </picture>',
