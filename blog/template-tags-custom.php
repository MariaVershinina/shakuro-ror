<?php
/**
 * Custom template tags for Storey
 *
 * @package Storey
 * @since 1.0
 */

if ( ! function_exists('storey_post_thumbnail') ) :
/**
 * Display the post thumbnail
 *
 * @return void
 */
function storey_post_thumbnail($postid) {
...
if ( ! function_exists( 'storey_get_image_description' ) ) :
/**
 ...
 * @return mixed
 */
function storey_get_image_description($p) {
  $image = get_post( $p );
...
if ( ! function_exists( 'storey_portfolio_media_feature' ) ) :
/**
 ...
 */
function storey_portfolio_media_feature($p, $meta) {
  $output = '<div class="portfolio-media-feature">';

    if( $meta['display_video'] ) {
      $output .= storey_print_video_html($p);
    } elseif( $meta['display_audio'] ) {
      $output .= storey_print_audio_html($p);
    } elseif( has_post_thumbnail($p) ) {
      //$output .= get_the_post_thumbnail($p, 'full');
      //$output .= storey_get_image_description( get_post_thumbnail_id( $p ) );
    }
...
if ( ! function_exists( 'storey_portfolio_media' ) ) :
/**
 ...
 */
function storey_portfolio_media($p, $meta) {
  $output = '<div class="portfolio-media-main">';
    ...
    if( $meta['display_video'] ) {
      ...
      if( $meta['display_gallery'] && $meta['display_audio'] ) {
        ...
        add_filter('storey_filter_gallery_content', 'storey_add_audio_to_gallery', 10, 2);
        // add gallery to output
        $output .= storey_post_gallery($p, 'portfolio-full', 'stacked');
      } elseif( $meta['display_audio'] ) {
...
function storey_add_audio_to_gallery($html, $postid) {
  return '<li>' . storey_print_audio_html($postid) . '</li>' . $html;
}

if ( !function_exists( 'storey_post_gallery' ) ) :
/**
 ...
 * @return void
 */
function storey_post_gallery( $postid, $imagesize = '', $layout = 'slideshow' ) {
...
if ( ! function_exists('storey_mod_video_shortcode') ) :
/**
 ...
 */
function storey_mod_video_shortcode($html, $atts, $video, $post_id, $library) {
...
add_filter('wp_video_shortcode', 'storey_mod_video_shortcode', 10, 5);


if ( ! function_exists('storey_post_title') ) :
/**
 * Display the post title
 *
 * @return void
 */
function storey_post_title() {
  if ( is_single() ) {
...
if( !function_exists('storey_post_header') ) :
/**
 * Print HTML header for current post
 *
 * @return void
 */
function storey_post_header() {
?>
  <!--BEGIN .entry-header-->
  <header class="entry-header">
  <?php if( !is_single() ) { ?>
    <?php if ( has_post_thumbnail() && !(has_post_format( 'video' ) || has_post_format( 'gallery' ) || has_post_format( 'audio' )) ) { ?>
      <div class="entry-thumbnail">
...
if( !function_exists('storey_post_footer') ) :
/**
 * Print HTML footer for current post
 *
 * @return void
 */
function storey_post_footer() {
?>
  <?php if( !is_single() ) { ?>
  <!--BEGIN .entry-meta -->
  <div class="entry-meta clearfix">
    <a href="<?php the_permalink(); ?>" class="zilla-button link small round"><?php _e( 'Keep Reading', 'zilla' ); ?></a>
    <div class="details">
...
if ( ! function_exists('storey_the_content') ) :
/**
 * Display the content
 *
 * @return  voide
 */
function storey_the_content() { ?>
...
if ( ! function_exists('storey_page_header' ) ) :
/**
 * Display page header
 *
 * @return void
 */
function storey_page_header() { ?>
  <header class="entry-header">
...
if ( ! function_exists( 'storey_paging_nav' ) ) :
function storey_paging_nav() {
...
if ( ! function_exists( 'storey_post_nav' ) ) :
/**
 * Display nav to prev/next post as needed
 *
 * @return  void
 */
function storey_post_nav() {
...
if ( ! function_exists( 'storey_portfolio_nav' ) ) :
/**
 * Print the portfolio navigation
 *
 * @return void
 */
function storey_portfolio_nav() { ?>
  <nav class="navigation single-post-navigation">
    <div class="nav-previous"><?php storey_previous_portfolio_link(); ?></div>
    <div class="nav-next"><?php storey_next_portfolio_link(); ?></div>
  </nav>
<?php }
endif;


if ( ! function_exists( 'storey_portfolio_meta' ) ) :
/**
 ...
 * @return void
 */
function storey_portfolio_meta($postid) {
  $output = '';
 ...
 */
function storey_get_adjacent_portfolio($previous = true) {
  global $wpdb;
 ...
 *
 * @uses  storey_get_adjacent_portfolio
 * @since  1.0
 * @return  void
 */
function storey_previous_portfolio_link() {
  $post = storey_get_adjacent_portfolio();
 ...
 * @return void
 */
function storey_next_portfolio_link() {
  $post = storey_get_adjacent_portfolio(false);
